    ...
    <div class="col-12">
        <div class="nav-tabs-custom">
            <ul class="nav nav-tabs">
                @foreach ($tabs as $tab)
                <li>
                    <a href="#{{ $tab }}" data-bs-toggle="tab" @if ($tab==$activeTab) class="active" @endif>
            ...
            <div class="tab-content">
                @foreach ($tabs as $tab)
                <div class="tab-pane @if ($activeTab == $tab) active @endif" id="{{ $tab }}">
                    @include('backend.admins.nas.includes.install.' . $tab)
                </div>
                @endforeach
                <!-- /.tab-pane -->
            </div>
