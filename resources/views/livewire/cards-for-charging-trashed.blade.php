<div x-data="{
    ...
    toggleAll() {
        ...
        if (this.isSelectAll) {
            this.multiselect = Array.from(document.querySelectorAll('input.cards-charging-trashed-checkbox:not(#cards_charging_trashed_all)')).map(cb => cb.value);
        } else {
            this.multiselect = [];
        }
    },
    ...
    <x-datatable :paginated-data="$paginatedData">
        <x-slot name="navBar">
            <div class="col-6">
                <h4><span class="badge badge-info fw-bold">سلة كروت الشحن</span></h4>
            </div>
                ...
                <th>
                    @if (session()->has('manager_login_key'))
                        <span class="h-20 flex-shrink-0 pull-left">
                            <input type="checkbox" id="cards_charging_trashed_all"
                                class="filled-in chk-col-success checkedId"
                                @click="toggleAll()">
                            <label for="cards_charging_trashed_all"></label>
                        </span>
                    @endif
            ...
            @if ($paginatedData && count($paginatedData) > 0)
                @foreach ($paginatedData as $index => $model)
                    <tr>
                        <td>
                            <div class="dropdown">
                                <div class="clearfix pull-left">
                                    @if (session()->has('manager_login_key'))
                                        <span class="h-20 flex-shrink-0 pull-left">
                                            <input type="checkbox" id="card_charge_trashed_{{ $model->id }}"
                                                value="{{ $model->id }}"
                                                class="filled-in chk-col-success cards-charging-trashed-checkbox deleted_users_table_checkbox"
                                                x-model="multiselect">
                                            <label for="card_charge_trashed_{{ $model->id }}"></label>
                                        </span>
                                    @endif
                        ...
                        <td><span class="badge badge-danger badge-pill">{{ $model->price }}</span></td>
                        <td><span class="badge badge-info badge-pill">{{ $model->admin_fullname ?? __('site.charging_with_admin') }}</span></td>
