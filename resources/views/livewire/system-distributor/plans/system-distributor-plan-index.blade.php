<x-datatable :paginated-data="$paginatedData">
    ...
    <x-slot name="navBar">
        <x-datatable.add-new :route="route('system.distributor.plans.create')" :title="__('datatable.add_new_plan')" />
        ...
        @if ($paginatedData && count($paginatedData) > 0)
            @foreach ($paginatedData as $index => $model)
                @if ($type == \App\ENUMS\PlanTypeEnum::TYPE_NETWORK)
                    <tr>
                        <td class="py-2">
                            @include('backend.system_distributor.plans.includes.optional_box')
                        </td>
                        <td>
