<x-datatable :paginated-data="$paginatedData">
    <x-slot name="navBar">
        <x-datatable.add-new :route="route('admins.cards.create')" :title="__('datatable.add_new_card_groups')" />
        ...
        @endisset
        @if (count($selectedModels) >= 1)
            <div>
                <button class="btn btn-danger col-sm-12 col-md-3 " wire:click="showWarningResetMacForCollectionBox()">
                    <i class="fa fa-undo text-primary px-2"></i>
                    {{ __('site.user_index.option.reset_mac_for_selected') }}
                </button>
            </div>
        @endif
    </x-slot>
    ...
    <x-slot name="thead">
        @if ($filterIsDisabled)
            <x-table-thead :columns="__('datatable.admin_cards_for_users_disabled')">
                <th>
            ...
            @foreach ($paginatedData as $index => $model)
                ...
                @else
                    <tr class="fw-bold bs-3 @if ($model->cards_count > 0) border-success @endif">
                        <td class="py-2">
                            @include('backend.optionalBox.cards-for-users')
                        </td>
                        <td class="no-padding">
                            <span
                                class="badge b-2 @if ($model->cards_count > 0) border-success
