<div>
    <x-datatable :paginated-data="$paginatedData">
            ...
            <div class="row pt-2 d-flex">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <button wire:click="$refresh" class="mx-2 btn btn-sm btn-outline btn-primary">
        ...
        <x-slot name="thead">
            <x-table-thead :columns="__('datatable.manager_transactions_paypal')">
            </x-table-thead>
        </x-slot>
        ...
        <x-slot name="tbody">
            ...
            <tr>
                ...
                <td class="no-padding">
                    <span class="badge  {{ $model->status == 'COMPLETED' ? 'badge-success' : 'badge-primary' }}">
                        {{ $model->gross_amount . ' ' . __('new_trans.dollar_code') ?? '---' }}
                    </span>
                </td>
                <td class="no-padding">
                    <span class="badge  {{ $model->status == 'COMPLETED' ? 'badge-success' : 'badge-primary' }}">
                        {{ $model->paypal_fee !== null ? $model->paypal_fee . ' ' . __('new_trans.dollar_code') : '---'}}
                    </span>
                </td>
                <td class="no-padding">
                    <span class="badge  {{ $model->status == 'COMPLETED' ? 'badge-success' : 'badge-primary' }}">
                        {{ $model->net_amount !== null ? $model->net_amount . ' ' . __('new_trans.dollar_code') : '---' }}
                    </span>
                </td>
                <td class="no-padding">
