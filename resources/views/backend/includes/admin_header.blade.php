<header class="main-header">
    <div class="d-flex align-items-center logo-box justify-content-start">
        <!-- Logo -->
        @include('backend.includes.manager_back')
        <a href="" class="logo">
            <!-- logo-->
    ...
    <nav class="navbar navbar-static-top">
        ...
        <div class="app-menu">
            <ul class="header-megamenu nav">
                ...
                <!-- <li class="d-block d-md-none" style="line-height: 2.9;">
        ...
        <div class="navbar-custom-menu r-side">
            <ul class="nav navbar-nav">
                <li class="d-none d-sm-block py-10">
                    <div>
                        @livewire('manager-back', ['manager_back_trans' => 'new_trans.return_to_system_dashboard'])
                    </div>
                </li>
                @if (config('networks.allow_blog.admin_search_questions', false))
                    <div x-data="{ showQuestionSearchModal: false }">
                                ...
                                <div class="search-bx mx-5">
                                    <div class="input-group">
                                        <input type="search" class="form-control"
                                            x-on:click="showQuestionSearchModal = true"
                        ...
                        <li class="btn-group d-lg-none d-inline-flex">
                            <button class="btn btn-dark py-2 mt-2 " type="button"
                                x-on:click="showQuestionSearchModal = true">
                ...
                @if (authIsAdmin() || authIsCafeAdmin())
                    <li class="btn-group d-lg-inline-flex d-none">
                        <div class="app-menu">
                            <div class="search-bx mx-5">
                                <span class="badge">
                ...
                <li class="dropdown " x-data="{ isOpen: false }">
                    <a href="#" class="waves-effect px-4 waves-light dropdown-toggle btn-primary-light "
                        @click="isOpen = !isOpen"
                        style="width:fit-content;font-size:small;display: inline-flex;align-items:center;">
                        ...
                        @if (App::getLocale() == 'ar')
                            ...
                            <div class=" text-sm px-1 xs-none"> {{ __('languages.locale.ar.name') }}
                            </div>
                        @else
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                width="30" zoomAndPan="magnify" viewBox="0 0 30 30.000001" height="40"
                            ...
                            <div class=" text-sm px-1 xs-none"> {{ __('languages.locale.en.name') }} </div>
                        @endif

                    </a>
                    <ul id='hidden_before_load' class=" animated flipInX px-0 text-center "
                        style="display:none;background-color:#112547;list-style:none;border-radius:5px;"
                                    ...
                                    <a class="dropdown-item" rel="alternate" hreflang="{{ $localeCode }}"
                                        ...
                                        <div class="inline-flex items-center"
                                            ...
                                            @if (App::getLocale() != 'ar')
                                                ...
                                                <div class="px-1">
                                                    <div class=" text-sm px-1"> {{ __('languages.locale.ar.name') }}
                                                    </div>
                                                </div>
                                            @else
                                                <svg xmlns="http://www.w3.org/2000/svg"
                                                ...
                                                <div class="px-1">
                                                    <div class=" text-sm px-1"> {{ __('languages.locale.en.name') }}
                                                    </div>
                                                </div>
                                            @endif
                                        </div>
                ...
                @include('backend.includes._chat_mute_toggle')
                <!-- Notifications -->
                <li class="dropdown notifications-menu">
                    <a href="#" class="waves-effect waves-light dropdown-toggle btn-primary-light"
                        ...
                        @if (auth('admin')->user()->unreadNotifications()->count() > 0)
                            <div class="badge badge-danger badge-pill text-white text-center"
                                style="width:20px;height: 20px;display:inline-flex; position:absolute;font-size:10px;top:0;right:0;z-index:2;">
                                <p>{{ auth('admin')->user()->unreadNotifications()->count() }}</p>
                            </div>
                        @endif
                    ...
                    <ul class="dropdown-menu animated bounceIn">
                        ...
                        <li class="header">
                            <div class="p-20">
                                <div class="flexbox">
                                    ...
                                    @if (auth('admin')->user()->unreadNotifications()->count() > 0)
                                        <div>
                                            <a style="font-size: 11px" href="{{ route('admins.mark_all_as_read') }}"
                                                class="text-danger">{{ __('notification.read_all') }}</a>
                                        </div>
                                    @endif
                                </div>
                            </div>
                        ...
                        <li>
                            ...
                            <ul class="menu sm-scrol" style="min-width: 250px;">
                                @forelse(auth('admin')->user()->unreadNotifications as $notification)
                                    <li>
                                        <a
                                            href="@if (isset($notification->data['route'])) {{ $notification->data['route'] }}@else javascript:void(0) @endif">
                                            <i class="fa fa-users text-info"></i>
                                            {{ $notification->data['message'] }}
                                            <span
                                                style="opacity: .5;font-size: 10px;width: 100%;display: block;text-align: left">{{ \Carbon\Carbon::parse($notification->created_at)->diffForHumans() }}</span>
                                        </a>
                                    </li>
                                @empty
                    ...
                    <ul class="dropdown-menu animated flipInX">
                        <li class="user-body">
                            <a class="dropdown-item"
                                href="{{ authIsCafeAdmin() ? route('cafe.settings.profile') : route('admins.settings.profile') }}">
                                <i class="ti-user text-muted me-2"></i>
                                @lang('website.navbar.admin_menu.profile')</a>
                            <a class="dropdown-item"
                                href="{{ authIsCafeAdmin() ? route('cafe.settings.index') : route('admins.settings.index') }}"><i
